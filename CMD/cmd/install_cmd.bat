...
set "ALIAS_FILE=stenocoder.bat"
set "EXE_FILE=stenocodercmd.exe"
set "TARGET_PATH=C:\Scripts\1TheCrazy\StenoCoder"
set "PATTERN=stenocodercmd-v*"
...
for %%F in ("%EXE_PATH%") do (
    ...
    set "CURRENT_VERSION=!CURRENT_VERSION:stenocodercmd-v=!"
    set "CURRENT_VERSION=!CURRENT_VERSION:.exe=!"
)

REM Check if the source files exist
if not exist "%SOURCE_PATH%" (
    echo The folder does not contain all contents necessary for the installation. Please download a new copy from https://github.com/1TheCrazy/StenoCoder.
    pause
    exit /b 1
)
...
if exist "%TARGET_PATH%" (
    ...
    for %%f in ("%TARGET_PATH%\%PATTERN%") do (
        set "filename=%%~nxf"
        set "version=!filename:stenocodercmd-v=!"
...
echo https://github.com/1TheCrazy/StenoCoder
pause
